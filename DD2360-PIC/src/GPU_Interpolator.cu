...
__global__ void interpP2G_kernel(struct GPUParticles* gpu_part, struct GPUInterpDensSpecies* gpu_ids, struct GPUGrid* gpu_grd) {  // TODO constant memory!
    ...
    __shared__ FPfield cell_vals_arr[32]; // 8 rho, 8 Jx, 8 Jy, 8 Jz
    ...
    if (threadIdx.x < 32) {  // first 8 threads zero shared memory
        cell_vals_arr[threadIdx.x] = 0;
    }
    ...
    for(long part_ind = threadIdx.x; part_ind < PART_SIZE_TEMP; part_ind += blockDim.x) {
        ...
        if (true) {  //(gpu_part->cell_id[part_ind] == (blockIdx.x + 1) * gpu_grd->nzc * gpu_grd->nyc + (blockIdx.y + 1) * gpu_grd->nzc + (blockIdx.z + 1)) {
            ...
            int subWarpId = laneId / 8;
            int subLaneId = laneId % 8;

            for(int i = 0; i < 2; i++)
                for(int j = 0; j < 2; j++)
                    for(int k = 0; k < 2; k++) {
                        // atomicAdd(&(cell_rho[i * 4 + j * 2 + k]), weight * xi[threadIdx.x][i] * eta[threadIdx.x][j] * zeta[threadIdx.x][k]);  // TODO thread has its own counter?
                        ...
                        cell_rho[i * 4 + j * 2 + k] += weight * xi[threadIdx.x][i] * eta[threadIdx.x][j] * zeta[threadIdx.x][k];
                        cell_Jx[i * 4 + j * 2 + k] += weight * xi[threadIdx.x][i] * eta[threadIdx.x][j] * zeta[threadIdx.x][k] * u[threadIdx.x];
                        cell_Jy[i * 4 + j * 2 + k] += weight * xi[threadIdx.x][i] * eta[threadIdx.x][j] * zeta[threadIdx.x][k] * v[threadIdx.x];
                        cell_Jz[i * 4 + j * 2 + k] += weight * xi[threadIdx.x][i] * eta[threadIdx.x][j] * zeta[threadIdx.x][k] * w[threadIdx.x];
                    }
        }
    }
